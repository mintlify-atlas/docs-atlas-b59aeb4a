---
title: "Broadcast Messages to All Chats"
...
- A failed send (e.g. the bot was removed from the group, or the user blocked the bot) logs a `BROADCAST_ERROR` entry in the error log and increments the `failed` count.
- Chats where delivery permanently fails are flagged with `send_failed = 1` in `chat_metadata`, and are surfaced in the dashboard so you can identify stale chats.
